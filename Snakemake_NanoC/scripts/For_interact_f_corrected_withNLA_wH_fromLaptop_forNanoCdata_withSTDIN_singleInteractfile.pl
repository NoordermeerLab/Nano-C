...
my $temppos = index($InputF, 'LC');
my $run = substr($InputF, $temppos, 4);
chomp $run;
...
for(my $m=0; $m<@filevpname; $m++)
{
	...
	my $outputfile = $run . "_" . $CHR . "_".  $vpname;

																											#Change the chromosome above
	print O1 "track type=interact name=\"$outputfile\" description=\"Nanopore 3C interactions\" maxHeightPixels=50:100:200 visibility=full\nbrowser position $CHR:1-$chr_len\n";
																											#Change the chromosome and its size above
