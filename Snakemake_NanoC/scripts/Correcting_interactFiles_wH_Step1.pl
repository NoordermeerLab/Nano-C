...
print "\n\nEnter the Interact file Directory\n";
my $dir = $ARGV[0];
chomp($dir);
...
my $output = $ARGV[1];
chomp($output);
open FO1, ">", $output or die;

foreach my $fp (glob("$dir/*.interact"))
{
	printf "%s\n", $fp;
	#open my $fh, "<", $fp or die "can't read open '$fp': ERROR";
	...
	print "Starting analyzing each file : \t$fp\n";
	my $InputF = $fp;
	chomp $InputF;

	my $prefix = $InputF;
	$prefix =~ s/\.interact//;
	my @name = split '\/', $prefix;
	my $OutputF = $prefix . "F_new.interact";
	print "\n$OutputF\n";
